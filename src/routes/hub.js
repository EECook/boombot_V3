...
router.get('/hub', requireAuth, async (req, res) => {
  try {
    const [members, extras, feedPosts] = await Promise.all([
      ...
      posts.listPosts({ wallUserId: null, viewerId: req.user.id })
    ]);
    ...
    const enrichedPosts = feedPosts.map(enrichPost);
    res.render('hub', { title: 'The Hub - Family Movie Night', roster, feedPosts: enrichedPosts });
  } catch (err) {
