...
const { PLATFORMS, getPlatform } = require('../config/platforms');
...
router.post('/profile/live', requireAuth, async (req, res) => {
  const isLive = req.body.isLive === true || req.body.isLive === 'true';
  const liveUrl = (req.body.liveUrl || '').trim().slice(0, 512);

  try {
    await profilesModel.setLiveStatus(req.user.id, isLive, liveUrl);
    const io = req.app.get('io');
    io.emit('profile:live', {
      discordId: req.user.discord_id,
      isLive,
      liveUrl: isLive ? liveUrl : null
    });
    res.json({ ok: true, isLive, liveUrl: isLive ? liveUrl : null });
  } catch (err) {
    console.error('setLiveStatus failed:', err.message);
    res.status(500).json({ error: 'Could not update your live status.' });
  }
});
...
router.get('/profile/:discordId', requireAuth, async (req, res) => {
  ...
  res.render('profile', {
    ...
    profile,
    links: links.map((l) => ({ ...l, meta: getPlatform(l.platform) })),
