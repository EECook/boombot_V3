...
/**
 * Fetches the full guild member roster using the BOT token, paginated
 * (Discord caps each request at 1000). Fine for a server this size.
 */
async function fetchGuildMembers() {
  ...
  while (true) {
    const { data } = await axios.get(`${DISCORD_API}/guilds/${DISCORD_GUILD_ID}/members`, {
      headers: { Authorization: `Bot ${DISCORD_BOT_TOKEN}` },
  ...
  // Drop bots from the roster - this is a members page, not a bot showcase
