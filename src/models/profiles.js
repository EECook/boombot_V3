...
async function updateProfile(userId, { age, pronouns, birthday, timezone, bio, customAvatarUrl }) {
  await pool.query(
    `UPDATE profiles SET
       age = ?, pronouns = ?, birthday = ?, timezone = ?, bio = ?, custom_avatar_url = ?
     WHERE user_id = ?`,
    [
      age || null,
      pronouns || null,
...
async function getRosterExtras() {
  const [users] = await pool.query(
    `SELECT u.discord_id, u.id AS user_id, p.bio, p.pronouns, p.custom_avatar_url
