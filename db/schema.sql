...
-- Safe to re-run: adds the live-status columns for installs that already had
-- the `profiles` table created before this feature existed.
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS is_live BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS live_url VARCHAR(512) DEFAULT NULL;
